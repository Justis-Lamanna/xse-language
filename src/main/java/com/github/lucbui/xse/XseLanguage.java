...
public class XseLanguage {
    ...
    public static final Language V1  = new LanguageBuilder()
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("alias")
					...
					.withParam(DirectiveParameter.TEXT, "Symbol")
					.withParam(DirectiveParameter.TEXT, "Alias"))
			...
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("braille")
					.withDescription("Braille text inserter. Same as the raw text one, except this will convert the text using the braille table. Note that braille letters are only uppercase")
					.withParam(DirectiveParameter.TEXT, "Text to be converted"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("break", "stop")
					.withDescription("When the compiler reaches this directive, it will stop processing the rest of the script. Useful for debugging"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("clean")
					.withDescription("Cleans the ROM from the last compiled script, as long as it was a dynamic one"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("define", "const")
					.withDescription("Allows you to define symbols which can replace numbers when compiling the script. Only numbers are allowed, but they can be any size, from byte to dword. It's good practice to use only uppercase letters as symbol names")
					.withParam(DirectiveParameter.TEXT, "Symbol name")
					.withParam(DirectiveParameter.NUMBER, "Value"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("definelist", "constlist")
					.withDescription("If the compiler log is enabled, the complete list of #defines used during the compilation will be shown"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("dynamic")
					.withDescription("Sets the dynamic start offset, or better, the offset the compiler will use to search free space for dynamic offsets")
					.withParam(DirectiveParameter.POINTER, "Pointer to begin searching for free space from"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("erase")
					.withDescription("Overwrites a certain amount of bytes with the free space byte, starting from the specified offset")
					.withParam(DirectiveParameter.POINTER, "Pointer to begin overwriting from")
					.withParam(DirectiveParameter.NUMBER, "Number of bytes to clear out"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("eraserange")
					.withDescription("Overwrites a specified range of bytes with the free space byte, starting from the specified offset and ending at the specified offset")
					.withParam(DirectiveParameter.POINTER, "Pointer to begin overwriting from")
					.withParam(DirectiveParameter.POINTER, "Pointer to end overwriting"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("freespace")
					.withDescription("Sets the free space-byte value used by #dynamic and/or #erase/#remove directives. The default value is 0xFF (255)")
					.withParam(new OptionalParameter(DirectiveParameter.ZERO_OR_FF), "Free space byte"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("include")
					.withDescription("Includes a header file in the compilation process. Double quotes are an aesthetic optional since they will be automatically removed. A header file is a file containing #defines/#aliases. Some header files already exist, eventually you can create your own. Remember that the header files must be placed in the folder where XSE is.")
					.withParam(DirectiveParameter.FILE, "Header filename"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("org", "seek")
					.withDescription("Tells the compiler where to start writing the script in the ROM. Here we have two choice. In fact, we can use either static or dynamic offsets. In the first case the specified location coincides with the offset used by the compiler; in the latter one the offset get calculated first and then assigned, depending on the free space available. The most obvious difference is that while static offsets have a real number, dynamic have an at symbol \"@\" followed by a dynamic label.")
					.withParam(DirectiveParameter.POINTER_OR_LABEL, "Location of the script"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("raw", "binary", "put")
					.withDescription("Inserts some raw data into the ROM. It can be used basically for movement data, Pokémon market items and level scripts. To determine which data type to use, simply add in the type's name before any values that follow. If you don't specify a particular type, the value will be treated as byte.")
					.withParam(new VarargsParameter(new OrParameter(DirectiveParameter.DATA_TYPE, DirectiveParameter.NUMBER)), "One or more bytes, words, or doubles, with types specified"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("remove")
					.withDescription("Removes the main part of a compiled script, if it's a valid one. Useful to gain free space and removing a spoilt script")
					.withParam(DirectiveParameter.POINTER, "Script to remove"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("removeall")
					.withDescription("Removes a compiled script, if it's a valid one, along with all his extra data, such as strings, movements, mart items and braille texts")
					.withParam(DirectiveParameter.POINTER, "Script to remove"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("removemart")
					.withDescription("Removes the mart data found at the specified offset")
					.withParam(DirectiveParameter.POINTER, "Mart data to remove"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("removemove")
					.withDescription("Removes the movements found at the specified offset")
					.withParam(DirectiveParameter.POINTER, "Movement data to remove"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("reserve")
					.withDescription("Reserves some bytes within any scripts. It can be useful if you plan to expand or to edit a script you compiled later on. The reserved bytes are simple nops so they won't affect the script itself")
					.withParam(DirectiveParameter.NUMBER, "Number of bytes to reserve"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("unalias")
					.withDescription("Removes an alias from the alias list")
					.withParam(DirectiveParameter.TEXT, "Alias to remove"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("unaliasall")
					.withDescription("Removes all aliases"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("undefine", "deconst")
					.withDescription("Removes a define from the define list, if any")
					.withParam(DirectiveParameter.TEXT, "Define to remove"))
			.withPreprocessingDirective(new PreprocessingDirectiveBuilder("undefineall", "deconstall")
					.withDescription("Removes all defines"))
			...
			.withCommand(new BasicCommandBuilder("bufferitems2")
					...
					.withParam(CommandParameter.ITEM_ID, "Item # to store")
					.withParam(CommandParameter.WORD, "Quantity"))
            /*To Decide: Do Macros belong here? Are there better ways to handle this builder?
			.withMacroCommand(new MacroCommandBuilder("msgbox")
					.withDescription("Loads a pointer into memory to display a message later on")
					.withParam(CommandParameter.POINTER, "Pointer to load into memory")
					.withParam(CommandParameter.BYTE, "Message type"))
			.withMacroCommand(new MacroCommandBuilder("message")
					.withDescription("Loads a pointer into memory to display a message later on")
					.withParam(CommandParameter.POINTER, "Pointer to load into memory")
					.withParam(CommandParameter.BYTE, "Message type"))
			.withMacroCommand(new MacroCommandBuilder("giveitem")
					.withDescription("Gives a specified item and displays an aftermath message of the player recieving the item")
					.withParam(CommandParameter.ITEM_ID, "Item # to add")
					.withParam(CommandParameter.WORD, "Quantity")
					.withParam(CommandParameter.BYTE, "Message type"))
			.withMacroCommand(new MacroCommandBuilder("giveitem2")
					.withDescription("Similar to giveitem, except playing a fanfare")
					.withParam(CommandParameter.ITEM_ID, "Item # to add")
					.withParam(CommandParameter.WORD, "Quantity")
					.withParam(CommandParameter.SOUND_ID, "Song # to play"))
			.withMacroCommand(new MacroCommandBuilder("giveitem3")
					.withDescription("Gives the player a specified decoration, and displays a related message")
                    .withParam(CommandParameter.DECORATION_ID, "Decoration #"))
            .withMacroCommand(new MacroCommandBuilder("wildbattle")
                    .withDescription("Starts a wild Pokémon battle")
                    .withParam(CommandParameter.POKEMON_ID, "Pokémon species to battle")
                    .withParam(CommandParameter.BYTE, "Level of the Pokémon")
                    .withParam(CommandParameter.ITEM_ID, "Item # to be held"))
            .withMacroCommand(new MacroCommandBuilder("wildbattle")
                    .withDescription("Starts a wild Pokémon battle using a specific graphic style")
                    .withParam(CommandParameter.POKEMON_ID, "Pokémon species to battle")
                    .withParam(CommandParameter.BYTE, "Level of the Pokémon")
                    .withParam(CommandParameter.ITEM_ID, "Item # to be held")
                    .withParam(CommandParameter.BYTE, "Battle style"))
            .withMacroCommand(new MacroCommandBuilder("registernav")
                    .withDescription("Register the specified trainer in the PokéNav. Emerald only")
                    .withParam(CommandParameter.WORD, "Trainer ID #"))*/
			.build();
